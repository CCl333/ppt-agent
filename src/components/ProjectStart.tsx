...
function renderSearchResult(
  ...
  const showRetry = Boolean(options?.onRetry) && Boolean(options?.allowRetry);
  ...
  return (
    <div key={source.id} className="rounded-[1.5rem] border border-slate-200 bg-white p-5 shadow-sm space-y-3">
      <div className="flex items-start justify-between gap-3">
        <div className="space-y-2 min-w-0">
          {isHttp ? (
            ...
            <div className="block text-base font-semibold text-slate-800 break-words">{source.title}</div>
          )}
      ...
      <p className="text-sm text-slate-600 leading-relaxed whitespace-pre-wrap break-words [overflow-wrap:anywhere]">
        {source.content_excerpt_md || source.bocha_summary || source.snippet || '已返回搜索摘要。'}
      </p>
      {showRetry ? (
...
export default function ProjectStart({
  ...
  const canConfirm = Boolean(pageCountAnswer) && Boolean(styleAnswer) && answeredAllQuestions && Boolean(form?.init_search_results?.length);
        ...
        <div className="flex-1 min-h-0 bg-white rounded-[2rem] shadow-sm border border-slate-200 flex flex-col overflow-hidden">
          <div className="px-6 py-5 border-b border-slate-100 space-y-4">
            <div className="flex items-center justify-between gap-4">
              <div>
                <div className="text-xs uppercase tracking-wide text-slate-400">联网资料</div>
            ...
            <div className="text-sm text-slate-500 leading-relaxed">
              初始化只保留搜索摘要，不抓取全文。确认需求后会用这些摘要生成大纲。
            </div>
          </div>
          ...
          <div className="border-t border-slate-100 p-4 bg-white space-y-3">
            ...
            <div className="flex items-center justify-between text-[11px] text-slate-400">
              <div className="flex items-center gap-1">
                <Database size={12} />
                搜索摘要 {form?.init_search_results.length ?? 0} 条
              </div>
              <div className="flex items-center gap-1">
                <Search size={12} />
